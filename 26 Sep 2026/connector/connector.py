...
def get_connector_extension_categories_snapshot(force_refresh: bool = False) -> List[Dict]:
    ...
    for row in application_rows:
        ...
        if not category_id or not category_name or not normalized:
            continue
        CATEGORY_CACHE["application"][normalized] = category_id
        merged[normalized] = {
            "category_id": category_id,
            "category_name": category_name,
            "category_source": "application",
            "application_category_id": category_id,
    ...
    for row in extension_rows:
        ...
        if existing:
            existing["extension_category_id"] = category_id
            existing["category_name"] = existing.get("category_name") or category_name
        else:
            merged[normalized] = {
                "category_id": category_id,
                "category_name": category_name,
                ...
                "extension_category_id": category_id,
            }

    snapshot = [merged[key] for key in sorted(merged, key=lambda item: merged[item]["category_name"].lower())]
...
def heartbeat():
    ...
    res = requests.post(f"{PORTAL_BASE_URL}/api/connector/heartbeat", headers=portal_headers(), json=payload, timeout=20)
    res.raise_for_status()
    print(f"Heartbeat OK: categories={len(extension_categories)}")


def fetch_job():
    payload = {"name": CONNECTOR_NAME, "host_os": CONNECTOR_HOST_OS, "extension_categories": CONNECTOR_CATEGORY_SNAPSHOT}
    res = requests.post(f"{PORTAL_BASE_URL}/api/connector/fetch-job", headers=portal_headers(), json=payload, timeout=30)
    res.raise_for_status()
    job = res.json().get("job")
    print(f"Fetch job OK: {'assigned' if job else 'none'}")
    return job


def send_result(job_id, status, result):
    payload = {"status": status, "result": result}
    res = requests.post(f"{PORTAL_BASE_URL}/api/connector/jobs/{job_id}/result", headers=portal_headers(), json=payload, timeout=30)
    res.raise_for_status()
    print(f"Result sent: job_id={job_id} status={status}")
...
def resolve_extension_category_id(category_name: str) -> Tuple[str, Dict]:
    ...
    if wanted in {"use team's extensions", "use teams extensions", "use team extensions", "team extensions", "follow team application", "team default"}:
        return "0", {"source": "team_default", "matched_name": clean_name}
    ...
    for row in extension_rows:
        ...
        if normalized and category_id:
            CATEGORY_CACHE["extension"][normalized] = category_id
    if wanted in CATEGORY_CACHE["extension"]:
        return CATEGORY_CACHE["extension"][wanted], {"source": "extension_category_fallback", "matched_name": clean_name}
    ...
    while True:
        try:
            ...
            if not job:
                ...
                continue
            print(f"Processing job: id={job['id']} profile={job.get('profile_name')}")
            try:
            ...
            print(f"Connector loop error: {type(exc).__name__}: {exc}")
            time.sleep(POLL_SECONDS)
