...
def build_review(project: Project, db: Session):
    ...
    for p in all_proxies:
        ...
        country, state, city = norm_geo(p.country_code, p.state_name, p.city_name)
        if country:
            proxies_by_geo_kind[(country, state, city, kind)].append(p)
        else:
    ...
    for job in all_jobs:
        key = (
            ...
            normalize_location_value(getattr(job, 'state_name', None)),
            normalize_location_value(getattr(job, 'city_name', None)),
        )
        queued_counts[key] += 1
        ...
        for analyst_id_str, share_count in shares.items():
            ...
            for device_type, os_type, count in device_chunks:
                ...
                for _ in range(count):
                    seq_key = (project.name, plan_country, plan_state, plan_city, analyst.name, os_type.upper())
                    seq_map[seq_key] += 1

                    candidates = []
                    if plan_city != "All":
                        candidates.extend([p for p in proxies_by_geo_kind.get((plan_country, plan_state, plan_city, required_proxy_kind), []) if p.id not in proxy_used_ids])
                    if plan_state != "All":
                        candidates.extend([p for p in proxies_by_geo_kind.get((plan_country, plan_state, "All", required_proxy_kind), []) if p.id not in proxy_used_ids])
                    candidates.extend([p for p in proxies_by_geo_kind.get((plan_country, "All", "All", required_proxy_kind), []) if p.id not in proxy_used_ids])
                    candidates.extend([p for p in untagged_by_kind.get(required_proxy_kind, []) if p.id not in proxy_used_ids])
                    proxy_obj = candidates[0] if candidates else None
                    proxy_reason = ""
                    if proxy_obj:
                    ...
                    payload = {
                        ...
                        "remark": project.remark_template or f"{project.name} | {analyst.name} | {location_text} | {os_type}",
...
def list_proxies(project_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    return [{
        ...
        "state_name": normalize_location_value(p.state_name),
        "city_name": normalize_location_value(p.city_name),
