        ...
        for analyst_id_str, share_count in shares.items():
            ...
            for device_type, os_type, count in device_chunks:
                ...
                for _ in range(count):
                    ...
                    profile_name = f"{normalize_name(project.name)}_{normalize_name(plan.country_code.upper())}_{normalize_name(analyst.name)}_{os_type.upper()}_{seq_map[seq_key]:03d}"
...
def update_project_settings(project_id: int, data: ProjectSettings, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    if not project:
        raise HTTPException(status_code=404, detail="Project not found")
    for field in ["desktop_pct", "mobile_pct", "windows_pct", "mac_pct", "android_pct", "iphone_pct", "ads_group", "extension_category", "default_proxy_type", "default_proxy_soft", "default_ipchecker", "connector_name", "remark_template"]:
...
def list_connectors(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    for c in connectors:
        ...
        age_seconds = None
        if c.last_seen:
            age_seconds = max(0.0, (utcnow() - c.last_seen).total_seconds())
        payload.append({
            "name": c.name,
            "host_os": c.host_os,
            "last_seen": c.last_seen.isoformat() if c.last_seen else None,
            "category_synced_at": c.category_synced_at.isoformat() if c.category_synced_at else None,
